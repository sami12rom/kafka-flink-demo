...
# Service Account
# --------------------------------------------------------
resource "confluent_service_account" "demo-sa" {
  display_name = "kafkaFlinkDemo-sa"
  description  = "Service Account for Kafka Flink Demo"
}
resource "confluent_role_binding" "demo-rb" {
  principal   = "User:${confluent_service_account.demo-sa.id}"
  role_name   = "EnvironmentAdmin"
  crn_pattern = confluent_environment.cc_demo_env.resource_name
}
output "cc_demo_sa" {
  description = "CC Service Account"
  value       = resource.confluent_service_account.demo-sa.id
}
